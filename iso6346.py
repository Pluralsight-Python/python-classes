...
def create(owner_code, serial, category='U'):
    ...
    full_code = raw_code + str(_check_digit(raw_code))
    return full_code


def _check_digit(raw_code):
    ...
    """
    s = sum(_code(char) * 2 ** index for index, char in enumerate(raw_code))
    return s % 11 % 10
...
def _code(char):
    ...
    """
    return int(char) if char.isdigit() else _letter_code(char)


def _letter_code(letter):
